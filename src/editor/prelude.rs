    ...
    pub fn load(filename: String) -> Result<Self, String> {
        ...
        for line in reader.lines() {
            if let Ok(mut line) = line {
                if line.len() >= 3
                    && line.as_bytes()[0] == 0xEF
                    && line.as_bytes()[1] == 0xBB
                    && line.as_bytes()[2] == 0xBF
                {
                    line = String::from_utf8(line.as_bytes().iter().skip(3).map(|b| *b).collect())
                        .unwrap();
                }
            ...
            file.write(
                if row_index + 1 != self.rows.len() {
                    format!(
                        "{}{}",
                        row.buf,
                        unescape(&config.line_ending).unwrap_or("\n".to_owned())
                    )
                } else {
                    format!("{}", row.buf)
                }
...
impl BinaryDocument {
    ...
    pub fn cursor_style() -> String {
        format!(
            "{}{}",
            crossterm::style::SetForegroundColor(crossterm::style::Color::from((255, 255, 51))),
...
impl BinaryDocument {
    pub fn new(filename: String) -> Self {
        Self {
            filename,
            ..BinaryDocument::default_doc()
        }
    }

    #[inline]
    pub const fn bytes_per_row() -> usize {
        32
    }
...
impl Document {
    pub fn display_name(&self) -> String {
        match self {
            Document::TextDocument(doc) => Path::new(&doc.filename)
                .file_name()
                .unwrap_or_default()
                .to_str()
                .unwrap_or_default()
                .to_owned(),
            Document::BinaryDocument(doc) => Path::new(&doc.filename)
                .file_name()
                .unwrap_or_default()
                .to_str()
                .unwrap_or_default()
    ...
    pub fn filename(&self) -> String {
        match self {
            Document::TextDocument(doc) => doc.filename.clone(),
            Document::BinaryDocument(doc) => doc.filename.clone(),
        }
    }

    pub fn dirty(&self) -> usize {
        match self {
            Document::TextDocument(doc) => doc.dirty,
            Document::BinaryDocument(doc) => doc.dirty,
        }
    }
    ...
    pub fn is_binary_doc(&self) -> bool {
        use Document::*;
        match self {
            BinaryDocument(_) => true,
            _ => false,
        }
    }
    ...
    pub fn as_bin_doc(&mut self) -> &mut BinaryDocument {
        match self {
            Document::BinaryDocument(doc) => doc,
            _ => panic!("Document isn't a text document!"),
        }
    }
}
...
impl Editor {
    ...
    pub fn add_doc(&mut self, doc: TextDocument) {
        self.docs.push(Document::TextDocument(doc));
        self.refresh_mouse_cache();
    }

    pub fn add_bin_doc(&mut self, doc: BinaryDocument) {
        self.docs.push(Document::BinaryDocument(doc));
        self.refresh_mouse_cache();
    }
    ...
    pub fn refresh_mouse_cache(&mut self) {
        ...
        for doc in &self.docs {
            self.docs_mouse_cache.push((
                i,
                i + doc.display_name().width() + 2 /*Side bars*/ + 2 /*Close icon*/ + 
                    if let Document::TextDocument(doc) = doc {
                        if doc.dirty == 0 {
                            0
                        }
                        else {
...
impl Editor {
    pub fn position_cursor(
        ...
        let mut i = 0;
        for c in rows[cursor_row]
            .buf
            .chars()
            .skip((cursor_col / width) * width)
        {
